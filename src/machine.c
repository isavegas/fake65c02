...
void write_memory(machine_t *machine, uint16_t address, uint8_t value) {
  ...
  if (machine->state.writable_rom && (address >= ROM_LOCATION && address < ROM_LOCATION + ROM_SIZE)) {
...
size_t load_rom(machine_t *machine, char *path) {
  ...
  mmap(ROM_SIZE, machine->rom, ROM_SIZE, PROT_READ | PROT_WRITE, MAP_PRIVATE, file, 0);
  fclose(file);
  #else
...
void free_machine(machine_t* machine) {
  if (machine->cpu != NULL) {
    free_fake65c02(machine->cpu);
    machine->cpu = NULL;
  }
  if (machine->ppu != NULL) {
    free_ppu(machine->ppu);
  }
  #ifndef __MINGW32__
  munmap(ROM_SIZE, machine->rom, ROM_SIZE)
  #endif
  free(machine);
}